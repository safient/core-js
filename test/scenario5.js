...
it('Should register a Guardian 3', async () => {
  ...
  it('Should create safe with "Testing Safe data" with Signal Based Claim', async () => {
      ...
      safeId = await creatorSc.createNewSafe(creator.idx.id, beneficiary.idx.id, "Testing safe Data", true, ClaimType.SignalBased, 10)
  ...
  it('Should send signal after claim', async () => {
      ...
      const result = await creatorSc.sendSignal(safeId) //Passing a claim
